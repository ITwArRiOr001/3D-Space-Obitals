...
const objectInfo = {
    ...
    milkyway: {
        ...
        moreInfoLink: "https://science.nasa.gov/resource/the-milky-way-galaxy/" // Updated URL
    }
};
...
function showInfoPanelFor(object) {
    ...
    if (!info) {
        console.error(`No info found for object: ${object.name}`);
        return;
    }
    ...
    if (infoLink) {
        infoLink.setAttribute('href', info.moreInfoLink);
        console.log(`Set href for ${info.name} to: ${info.moreInfoLink}`); // Debug log
        // Ensure default link behavior
        infoLink.onclick = function(event) {
            console.log(`Clicked More Info link for ${info.name}: ${info.moreInfoLink}`); // Debug click
            window.open(info.moreInfoLink, '_blank'); // Force open in new tab
            event.stopPropagation(); // Prevent interference from parent elements
        };
    } else {
        console.error('info-link element not found in DOM');
    }
